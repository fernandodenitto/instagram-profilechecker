...
def addItems(users, data, real):
    ...
    for user in users:
        if user["username"] not in ignoreList:
            ...
            # Biography present
            item["biography"] = 0 if user["biography"] == "" else len(user["biography"])
            # Follows count
            ...
            # Recently removed
            item["recently_removed"] = True if user["username"] in removedList else False
            ...
            if user["username"] in removedList:
                item["average_likes"] = 0
                item["max_likes"] = 0
                item["min_likes"] = 0            
                item["std_likes"] = 0
                item["var_likes"] = 0
                ...
                item["mean_time_between_posts"] = 0
                item["max_time_between_posts"] = 0
                item["min_time_between_posts"] = 0
                item["std_time_between_posts"] = 0
                item["var_time_between_posts"] = 0
                item["skw_time_between_posts"] = 0
            else:
                if os.path.isfile("users_posts/"+directory + username + ".json"):
                    with open("users_posts/"+directory + username + ".json", encoding="utf8") as t:
                        ...
                        item["std_likes"] = 0 if len(likes) < 2 else statistics.stdev(likes)
                        item["var_likes"] = 0 if len(likes) < 2 else statistics.variance(likes)
                        ...
                        item["std_comments"] = 0 if len(comments) < 2 else statistics.stdev(comments)
                        item["var_comments"] = 0 if len(comments) < 2 else statistics.variance(comments)
                        item["skw_comments"] = skew(comments)

                        dates.reverse()
                        times = [dates[i + 1] - dates[i] for i in range(len(dates)-1)]
                        
                        item["mean_time_between_posts"] = 0 if len(times) == 0 else statistics.mean(times)
                        item["max_time_between_posts"] = 0 if len(times) == 0 else max(times)
                        item["min_time_between_posts"] = 0 if len(times) == 0 else min(times) 
                        item["std_time_between_posts"] = 0 if len(times) < 2 else statistics.stdev(times)
                        item["var_time_between_posts"] = 0 if len(times) < 2 else statistics.variance(times)
                        item["skw_time_between_posts"] = 0 if len(times) == 0 else skew(times)
                else:
